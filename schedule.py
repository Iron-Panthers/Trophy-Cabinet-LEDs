...
HEADER = '''/* 
 * This is an autogenerated file created on {datetime.now()} 
...
array = ',\n  '.join(f'LEDDuration{{{int(d.start.timestamp())},{int(d.end.timestamp() - d.start.timestamp())}}}' for d in durations)
...
with open('trophy_cabinet/schedule.h', 'w') as f:
	f.write(HEADER)
	f.write(f'// LEDs are on for {total_duration / 3600} hours per cycle\n\n')
